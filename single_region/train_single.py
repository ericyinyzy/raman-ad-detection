...
for recur in range(1):
    ...
    while epoch<max_epoch:
            ...
            if epoch%3==0:
                ...
                precision = pos_correct_correct/total_pred_pos if total_pred_pos>0 else 0
                recall = pos_correct_correct/pos_all_correct if pos_all_correct>0 else 0
...
background = background.unsqueeze(0).to('cpu')#.numpy()
...
# 转换为 NumPy 数组
vector_np = background.squeeze().numpy()
...
for batch in tqdm(test_dl, desc="Collecting test data"):
    ...
    label = batch['label'][:, 0].cpu().numpy()
    all_labels.append(label)
all_test_inputs = torch.cat(allputs, dim=0).cpu()  # shape: (num_test_samples, 701)
# all_test_inputs=
...
for feature_idx in range(701):
    # 克隆原始输入
    masked_input = all_test_inputs.to(device)
